...
typedef NS_ENUM(NSUInteger, HPageControlImageType) {
	HPageControlImageTypeNormal = 1,
	HPageControlImageTypeCurrent,
	HPageControlImageTypeMask
};
...
- (void)_initialize
{
	...
	_alignment = HPageControlAlignmentCenter;
	_verticalAlignment = HPageControlVerticalAlignmentMiddle;
...
- (CGFloat)_leftOffset
{
	...
	CGFloat left = 0.0f;
	switch (_alignment) {
		case HPageControlAlignmentCenter:
			left = CGRectGetMidX(rect) - (size.width / 2.0f);
			break;
		case HPageControlAlignmentRight:
...
- (CGFloat)_topOffsetForHeight:(CGFloat)height rect:(CGRect)rect
{
	...
	switch (_verticalAlignment) {
		case HPageControlVerticalAlignmentMiddle:
			top = CGRectGetMidY(rect) - (height / 2.0f);
			break;
		case HPageControlVerticalAlignmentBottom:
...
- (void)_setImage:(UIImage *)image forPage:(NSInteger)pageIndex type:(HPageControlImageType)type
{
	if (pageIndex < 0 || pageIndex >= _numberOfPages) {
		return;
	}
	...
	switch (type) {
		case HPageControlImageTypeCurrent:
			dictionary = self.currentPageImages;
			break;
		case HPageControlImageTypeNormal:
			dictionary = self.pageImages;
			break;
		case HPageControlImageTypeMask:
...
{
    [self _setImage:image forPage:pageIndex type:HPageControlImageTypeNormal];
	[self _updateMeasuredIndicatorSizes];
}

- (void)setCurrentImage:(UIImage *)image forPage:(NSInteger)pageIndex
{
	[self _setImage:image forPage:pageIndex type:HPageControlImageTypeCurrent];;
	[self _updateMeasuredIndicatorSizes];
}

- (void)setImageMask:(UIImage *)image forPage:(NSInteger)pageIndex
{
	[self _setImage:image forPage:pageIndex type:HPageControlImageTypeMask];
...
- (id)_imageForPage:(NSInteger)pageIndex type:(HPageControlImageType)type
{
	if (pageIndex < 0 || pageIndex >= _numberOfPages) {
		return nil;
	}
...
{
	return [self _imageForPage:pageIndex type:HPageControlImageTypeNormal];
}

- (UIImage *)currentImageForPage:(NSInteger)pageIndex
{
	return [self _imageForPage:pageIndex type:HPageControlImageTypeCurrent];
}

- (UIImage *)imageMaskForPage:(NSInteger)pageIndex
{
	return [self _imageForPage:pageIndex type:HPageControlImageTypeMask];
}
